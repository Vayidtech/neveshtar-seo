...
export function SeoStudio() {
  ...
  const [tab, setTab] = useState<"preview" | "html" | "text">("html");
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-xl border border-border bg-surface p-5 sm:p-6">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <h2 className="flex items-center gap-2 text-sm font-medium text-fg"><FileText className="size-4 text-muted" /> خروجی HTML برای سایت</h2>
              <div className="flex gap-1 rounded-md border border-border bg-bg p-1">
                {([["html", "HTML کپی"], ["preview", "پیش‌نمایش"], ["text", "متن"]] as const).map(([id, label]) => (
            ...
            <p className="mb-2 text-xs text-muted">
              این HTML شامل تگ‌های واقعی h1 / h2 / h3 و img است. مستقیم در ویرایشگر سایت بچسبانید.
            </p>

            {tab === "preview" && (
              <div className="max-h-[32rem] overflow-y-auto rounded-md border border-border bg-white p-4 text-sm text-black [&_h1]:text-xl [&_h1]:font-bold [&_h1]:my-4 [&_h2]:text-lg [&_h2]:font-semibold [&_h2]:my-3 [&_h3]:text-base [&_h3]:font-semibold [&_h3]:my-2 [&_img]:max-w-full [&_img]:my-3" dir="rtl"
          ...
          <section className="rounded-xl border border-border bg-surface p-5 sm:p-6">
            <h2 className="mb-4 flex items-center gap-2 text-sm font-medium text-fg"><ImageIcon className="size-4 text-muted" /> تصاویر (داخل HTML هم درج شده‌اند)</h2>
            <div className="grid gap-4 sm:grid-cols-3">
